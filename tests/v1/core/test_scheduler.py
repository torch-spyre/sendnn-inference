...
class TestUpdateAfterScheduleBumpsPlaceholders:
    ...
    def test_last_prefill_chunk_is_also_tracked(self, sync_scheduler):
        """The final chunk that completes the prefill MUST also be tracked.

        Regression test for an earlier version that gated the bump on
        ``request.is_prefill_chunk``. The base scheduler clears that flag
        for the last chunk (since ``num_computed`` reaches ``num_tokens``
        exactly), so gating on it would miss the last chunk's advance.
        That caused two downstream bugs: (a) the previous chunk's
        ``committed_before`` was miscomputed during reconciliation, and
        (b) the request was prematurely promoted to decode while the final
        chunk was still in flight, tripping ``_prepare_decode``'s
        ``input_batch`` size assertion.
        """
        chunk_size = sync_scheduler.chunk_size
        prompt_len = chunk_size  # exactly one chunk -> last chunk
        ...
        def fake_base_update_after(self_inner, sched_out):
            for rid, n in sched_out.num_scheduled_tokens.items():
                r = self_inner.requests[rid]
                r.num_computed_tokens += n
                # Mirror the base scheduler clearing is_prefill_chunk when
                # num_computed reaches num_tokens.
                r.is_prefill_chunk = r.num_computed_tokens < (
                    r.num_tokens + r.num_output_placeholders
                )
        ...
        # Last chunk must still be tracked: prior_optimistic (0) < prompt_len.
        assert req.num_computed_tokens == chunk_size
        assert req.is_prefill_chunk is False  # base scheduler cleared it
        assert sync_scheduler._inflight_prefill_tokens["req1"] == chunk_size
...
class TestChunkedPrefillSpyreSchedulerUpdateFromOutput:
    ...
    def test_async_partial_execution_passes_through(self, async_scheduler):
        """Partial execution shouldn't normally happen with
        ``step_with_batch_queue``, but if it does we pass the original
        ``scheduler_output`` through unmodified rather than mutate it
        (which would risk leaving other fields inconsistent)."""
        ...
        # The unmodified scheduler output is forwarded.
    ...
    def test_request_not_promoted_to_decode_while_last_chunk_in_flight(self, async_scheduler):
        ...
        decode, and the worker's ``_prepare_decode`` fails its assertion
        ``len(input_batch.req_id_to_index) == len(cached_request_data.req_ids)``
        because the runner only registers the request in ``input_batch``
        when the final chunk physically executes (via
        ``_maybe_prepare_last_prefill``).
        """
        chunk_size = async_scheduler.chunk_size
        chunk_1_n = chunk_size
        chunk_2_n = 72
        prompt_len = chunk_1_n + chunk_2_n  # 200

        req = _make_request("req1", prompt_len=prompt_len)
        # Simulate post-_update_after_schedule for two run-ahead chunks: the
        # final chunk MUST be tracked, so inflight covers both chunks.
        req.num_computed_tokens = prompt_len
        async_scheduler._inflight_prefill_tokens["req1"] = chunk_1_n + chunk_2_n
        async_scheduler.requests["req1"] = req
        async_scheduler.running = [req]
        ...
        # Chunk 2 is still in flight; the request MUST still appear in
        # ongoing_prefills on the next schedule() call so it is not
        # promoted to decode prematurely.
        with patch(
            "vllm.v1.core.sched.async_scheduler.AsyncScheduler.schedule",
            return_value=_make_scheduler_output([], tokens_per_req=0),
        ):
